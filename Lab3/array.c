...
int bSearch(int *arr, int a, int b, int key){
    ...
    if (b >= a) {
      ...
      if (*(arr+mid) > key)  // If element is smaller than mid, then it can only be present in left subarray
            return bSearch(arr, a, mid - 1, key);

        return bSearch(arr, mid + 1, b, key);  // Else the element can only be present in right subarray
    }
